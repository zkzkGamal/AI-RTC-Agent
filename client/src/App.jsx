/**
 * Root React component: wires up the RTC session UI, controls, status, and transcript.
 */

import { useState, useRef, useCallback, useEffect } from 'react';
import './App.css';
import { createSession, sendOffer, sendChatMessage, getUserSessions, getSessionMessages, uploadCv } from './services/api';
import { createConnection, applyAnswer, closeConnection } from './services/webrtc';
import { connectSocket, joinSession, listenToTools, disconnectSocket } from './services/socket';
  ...
  useEffect(() => {
    ...
    const loadSessionDetails = async () => {
      try {
        ...
        const filtered = data.messages.filter(m => (m.role === 'human' || m.role === 'ai') && m.content && m.content.trim()).map(m => ({
  ...
  const handleCvSelected = async e => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: 'cv-ack-' + Date.now(),
        sender: 'agent',
        text: `✅ CV processed: ${profile.file_name}.` + (profile.summary ? `\n${profile.summary}` : '') + (kw ? `\nKey skills: ${kw}` : ''),
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      const {
        ...
        stream,
        offer
      } = await createConnection(handleConnectionState, handleMessage);
      pcRef.current = pc;
        ...
        <div className="left-panel-column">
          ...
          <section className="agent-card settings-panel">
            ...
            <div className="user-settings">
              {isEditingUser ? <form onSubmit={handleSaveUserId} className="user-id-form">
                  <input type="text" value={userInputVal} onChange={e => setUserInputVal(e.target.value)} placeholder="Enter User ID..." className="text-input font-mono" maxLength={30} autoFocus />
                ...
                </form> : <div className="user-id-display">
          ...
          <section className="agent-card control-panel">
            ...
            <StatusDisplay status={status} statusMsg={statusMsg} sessionId={webrtcSessionId} error={error} />
            
            <ControlButtons onStart={handleStart} onStop={handleStop} isConnected={isConnected} isBusy={isBusy} />
          </section>
          ...
          <section className="agent-card history-panel">
            ...
            <div className="sessions-list">
              {sessionsList.length === 0 ? <div className="empty-sessions">
                  <p>No past sessions found</p>
                </div> : sessionsList.map(s => <div key={s.session_id} className={`session-item font-mono ${agentSessionId === s.session_id ? 'active' : ''}`} onClick={() => setAgentSessionId(s.session_id)}>
        ...
        <section className="agent-card timeline-panel flex-grow-timeline">
          <div className="section-header">
            ...
            <span className={`badge status-badge ${isConnected && !isAgentThinking ? 'active' : ''}`}>
              {isConnected ? isAgentThinking ? '🔇 MIC MUTED (AGENT RESPONDING)' : '🎙️ MIC ACTIVE (SPEAK NOW)' : '🔇 MIC INACTIVE'}
            </span>
          </div>

          <div className="transcripts-timeline">
            {messages.length === 0 ? <div className="empty-timeline">
                ...
                {messages.map(m => <div key={m.id} className={`timeline-item bubble-${m.sender} animate-slide-in`}>
                    ...
                    <div className="item-content">
                      {m.sender === 'agent' ? <Markdown text={m.text} /> : <p>{m.text}</p>}
                    </div>
                  </div>)}

                {}
                {isAgentThinking && <div className="timeline-item bubble-agent animate-slide-in">
                    <div className="item-header">
                    ...
                    <div className="item-content thinking-content">
                      <div className="thinking-row">
                        ...
                        {runningTool && <div className="running-tool-badge font-mono animate-pulse">
                ...
                {}
                {sessionPendingConfirmation && <div className="hil-confirmation-card animate-slide-in">
                    ...
                    <div className="hil-feedback-section">
                      ...
                      <div className="feedback-input-row">
                        <input id="hil-feedback-input" type="text" value={feedbackText} onChange={e => setFeedbackText(e.target.value)} placeholder="Type changes and submit..." className="text-input" />
                        <button className="btn btn-submit-feedback" onClick={() => {
          ...
          <form onSubmit={handleSendText} className="chat-composer-form">
            <input ref={cvInputRef} type="file" accept=".pdf,.doc,.docx,.md,.markdown,.txt" onChange={handleCvSelected} style={{
            display: 'none'
          }} />
            <button type="button" className="chat-composer-submit" onClick={() => cvInputRef.current?.click()} disabled={isAgentThinking || isUploadingCv} title="Upload CV (PDF, Word, or Markdown)">
              {isUploadingCv ? '⏳' : '📎 CV'}
            </button>
            <input type="text" value={typedMessage} onChange={e => setTypedMessage(e.target.value)} placeholder={sessionPendingConfirmation ? "Please confirm or reject the pending action..." : "Type candidate response..."} className="chat-composer-input" disabled={isAgentThinking} />
            <button type="submit" className="chat-composer-submit" disabled={isAgentThinking || !typedMessage.trim()}>
              Send
            </button>
          </form>
        </section>
